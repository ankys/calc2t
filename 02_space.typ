...
#block[
_Proof.]
有界閉矩形$[a_1, b_1] times dots times [a_N, b_N]$で被覆できたとすると\$L = \\max{ |a\_1|, |b\_1|, \\cdots, |a\_N|, |b\_N| }\$として有界閉矩形$[- L, L]^N$でも被覆でき、$R = sqrt(N) L$とすればよい。
...
このとき、$i = 1, dots, N$に対して以下で$a^(‾)_i, b^(‾)_i$を定めると、
$[a^(‾)_1, b^(‾)_1] times dots times [a^(‾)_N, b^(‾)_N]$は有界閉矩形であり、$X subset [a^(‾)_1, b^(‾)_1] times dots times [a^(‾)_N, b^(‾)_N]$であり、$X subset [a_1, b_1] times dots times [a_N, b_N]$ならば$[a^(‾)_1, b^(‾)_1] times dots times [a^(‾)_N, b^(‾)_N] subset [a_1, b_1] times dots times [a_N, b_N]$を満たす。
\$\$\\bar{a}\_i = \\sup{ a in RR mid(|) X \\subset { x in RR^N mid(|) x\_i >= a } },
quad
\\bar{b}\_i = inf{ b in RR mid(|) X \\subset { x in RR^N mid(|) x\_i <= b } }.\$\$
...
#block[
点列$(a_n)_(n in bb(N))$が$n arrow.r oo$で点$a$に_収束する_あるいは$a$が$(a_n)_(n in bb(N))$の$n arrow.r oo$での_極限である_とは$n arrow.r oo$で
$ d(a, a_n) arrow.r 0 $ が成り立つことをいう。
点列$(a_n)_(n in bb(N))$が何らかの数に収束する時この点列は_収束する_といい、そうでない時_発散する_という。
...
== 連続関数
<連続関数>
この節では主に多変数関数$f : RR times dots times RR arrow.r RR$の極限や連続性について見ていく。
...
より詳しくは$N = 1, 2, 3, dots$として、$N$次元点集合$X$上で定義された実数値関数$f : X arrow.r RR$を考える。
...
関数$f (x)$が$x arrow.r a$で実数$l$に_収束する_あるいは$l$が$f (x)$の$x arrow.r a$での_極限である_とは
連続性の度合い$omega_a$が存在し任意の$x in X$に対して
$ lr(|f (x)-l|) <= omega_a (d(a, x)) $ が成り立つことをいう。

]
今後の議論の展開の中で値域が実数でなく$RR^M$
($M = 1, 2, 3, dots$)である場合に拡張する必要がありそのために極限の概念を拡張しておく。

#block[
$F$を点集合$X$上の$RR^M$値関数、$a$を$X$の極限点、$L$を$RR^M$の点とする。
関数$F (x)$が$x arrow.r a$で点$L$に_収束する_あるいは$L$が$F (x)$の$x arrow.r a$での_極限である_とは
連続性の度合い$omega_a$が存在し任意の$x in X$に対して
$ d(L, F (x)) <= omega_a (d(a, x)) $ が成り立つことをいう。
...
各$i = 1, dots, M$に対して関数$f_i (x)$が$x arrow.r a$で実数$l_i$に収束することと同値である。

]
#block[
_Proof.] $F (x)$が収束するならばの方が簡単で、
$ lr(|f_i (x)-l_i|) <= d(L, F (x)) <= omega_a (d(a, x)) $
であることから直ちに従う。

各$f_i (x)$が収束する場合は
$ lr(|f_i (x)-l_i|) <= omega_a^i (d(a, x)) $
となる連続性の度合い$omega_a^i$が存在し、
新しい連続性の度合いを\$\\omega\_a(r) = \\max{ \\omega\_a^1(r), \\cdots, \\omega\_a^M(r) }\$として取ると、
$ d(L, F (x)) = sqrt((f_1 (x)-l_1)^2+dots+(f_M (x)-l_M)^2) <= sqrt(omega_a^1 (d(a, x))^2+dots+omega_a^M (d(a, x))^2) <= sqrt(M) omega_a (d(a, x)) $
なので$F (x)$も収束する。~◻
...
$N$変数関数$f (x_1, dots, x_N)$が$(x_1, dots, x_N) arrow.r (a_1, dots, a_N)$で実数$l$に_収束する_あるいは$l$が$f (x_1, dots, x_N)$の$(x_1, dots, x_N) arrow.r (a_1, dots, a_N)$での_極限である_とは
連続性の度合い$omega_((a_1, dots, a_N))$が存在し任意の$(x_1, dots, x_N) in X$に対して
$ lr(|f (x_1, dots, x_N)-l|) <= omega_((a_1, dots, a_N)) (sqrt((x_1-a_1)^2+dots+(x_N-a_N)^2)) $
が成り立つことをいう。
...
ここで$f (x_1, dots, x_N)$が$(x_1, dots, x_N) arrow.r (a_1, dots, a_N)$で$f (a_1, dots, a_N)$に収束する、つまり
$ lim_((x_1, dots, x_N) arrow.r (a_1, dots, a_N)) f (x_1, dots, x_N) = f (a_1, dots, a_N) $
...
#block[
以上のことをまとめると$N$変数実数値関数$f (x_1, dots, x_N)$が$(x_1, dots, x_N) = (a_1, dots, a_N)$で連続であることは連続性の度合い$omega_((a_1, dots, a_N))$が存在し
$ lr(|f (x_1, dots, x_N)-f (a_1, dots, a_N)|) <= omega_((a_1, dots, a_N)) (sqrt((x_1-a_1)^2+dots+(x_N-a_N)^2)) $
が成り立つことと同値である。
...
\$\$\\proj\_i: x\_1 in RR, \\cdots, x\_N in RR \\mapsto x\_i in RR.\$\$

]
#block[
_Proof.] 次の不等式が成り立つことからすぐわかる。
$ lr(|x_i-a_i|) <= sqrt((x_1-a_1)^2+dots+(x_N-a_N)^2) . $~◻
...
#block[
_Proof.] 三角不等式より
$ lr(|f (x)-f (a)|) = lr(|d(hat(a), x)-d(hat(a), a)|) <= d(a, x) $
なので、$f$は連続関数である。~◻
...
実際、三角不等式より
$ lr(|d(x, y)-d(a, b)|) = lr(|(d(x, y)-d(a, y))+(d(a, y)-d(a, b))|) <= d(a, x)+d(b, y) <= 2 sqrt(d(a, x)^2+d(b, y)^2) $
...
この時、ある$epsilon > 0$が存在して、任意の$delta > 0$に対して$d(a, x) < delta$かつ$lr(|f (x)-f (a)|) >= epsilon$となる点$x$が存在する。
...
一方で$lr(|f (a_n)-f (a)|) >= epsilon$より$(f (a_n))$は$f (a)$に収束しないので仮定に矛盾する、よって$f (x)$は$x = a$で連続である。~◻
...
+ $+ : x in RR, y in RR arrow.r.bar x+y in RR$.

+ $- : x in RR, y in RR arrow.r.bar x-y in RR$.

+ $times : x in RR, y in RR arrow.r.bar x y in RR$.

+ \$\\divs: x in RR, y in RR\\setminus{0} \\mapsto \\frac{x}{y} in RR\$.

+ \$\\max: x in RR, y in RR \\mapsto \\max{ x, y } in RR\$.

+ \$\\min: x in RR, y in RR \\mapsto \\min{ x, y } in RR\$.

]
#block[
_Proof.] 定義域から点$(a, b)$を固定する。

+ 三角不等式より
  $ lr(|(x+y)-(a+b)|) = lr(|(x-a)+(y-b)|) <= lr(|x-a|)+lr(|y-b|) <= 2 sqrt((x-a)^2+(y-b)^2) . $
...
+ 三角不等式より$r = sqrt((x-a)^2+(y-b)^2)$とおくと
  $ lr(|x y-a b|) = lr(|(x y-x b)+(x b-a b)|) <= lr(|x|) lr(|y-b|)+lr(|x-a|) lr(|b|) <= lr(|x-a|) lr(|y-b|)+lr(|a|) lr(|y-b|)+lr(|x-a|) lr(|b|) <= r^2+(lr(|a|)+lr(|b|)) r . $
  ここで、各$(a, b)$ごとに$lim_(r arrow.r 0 +) (r^2+(lr(|a|)+lr(|b|)) r) = 0$より、乗法$times$は$(a, b)$で連続であり、$RR times RR$上の連続関数である。

+ 最初に$(a, b)$に十分近い$(x, y)$のみ考えることで$sqrt((x-a)^2+(y-b)^2) < lr(|b|) / 2$、特に$lr(|y-b|) <= lr(|b|) / 2$で$lr(|y|) >= lr(|b|) / 2$としてよいことに注意する。
  三角不等式より$r = sqrt((x-a)^2+(y-b)^2)$とおくと
  $ lr(|x / y-a / b|) = frac(lr(|x b-a y|), lr(|b|) lr(|y|)) = frac(lr(|(x b-x y)+(x y-a y)|), lr(|b|) lr(|y|)) <= frac(lr(|x|) lr(|y-b|), lr(|b|) lr(|y|))+lr(|x-a|) / lr(|b|) <= 2 frac(r^2+lr(|b|) r, lr(|b|)^2)+r / lr(|b|) . $
  ここで、各$(a, b)$ごとに$lim_(r arrow.r 0 +) (2 frac(r^2+lr(|b|) r, lr(|b|)^2)+r / lr(|b|)) = 0$より、除法\$\\divs\$は$(a, b)$で連続であり、\$RR\\times(RR\\setminus{0})\$上の連続関数である。

+ 一般に \$\$\\max{ x, y } = \\frac{x+y}{2}+\\frac{|y-x|}{2}\$\$
  が成り立ち、連続関数の合成になっているので、最大$max$は$RR times RR$上の連続関数である。

+ 一般に \$\$\\min{ x, y } = \\frac{x+y}{2}-\\frac{|y-x|}{2}\$\$
...
#block[
$(a, b) in RR times RR$として、$f$を\$(RR\\timesRR)\\setminus{(a, b)}\$上で定義された二変数実数値関数とする。
...
を考える。 このとき
$ lr(|f (r cos theta, r sin theta)-f (0, 0)|) = lr(|r^2 cos theta sin theta|) / r = r lr(|cos theta sin theta|) <= r arrow.r 0 quad(r arrow.r 0) . $
...
($a_n in RR^N$)に対して極限$a$が$A$の元であるとき、$A$は_閉集合_であるという。
...
このとき、$f (a_n) <= 0$であり、$f$が連続関数なので$f (a_n) arrow.r f (a)$であることから、$f (a) <= 0$が従う。
...
- $A$, $B$が開集合として、共通部分$A sect B$を考える。
  ...
  よって\$r = \\min{ r\_A, r\_B } \> 0\$とおくと、$B_r (a) subset A sect B$となるので、$A sect B$は開集合である。
...
#block[
_Proof.]
有界閉矩形$[a_1, b_1] times dots times [a_N, b_N]$上の点列$(x_n) = ((x_n^1, dots, x_n^N))$が$x = (x^1, dots, x^N)$に収束したとすると、各$i = 1, dots, N$に対して$a_i <= x_n^i <= b_i$なので$a_i <= x^i <= b_i$、つまり極限$x = (x^1, dots, x^N)$は$[a_1, b_1] times dots times [a_N, b_N]$の元である。
...
#block[
_Proof.] 最大について示す。
$f$の$X$上の上限を$l in RR^(‾)$とおくと、$X$が空集合でないので$l eq.not-oo$であり、$f (a_n) arrow.r l$となる$X$上の点列$(a_n)$が存在する。
...
ここで$f$と$c$の合成関数$f (c (t)) : [0, 1] arrow.r RR$は連続関数であり$f (c (0)) = f (x)$,
...
#block[
<t_pcurv>
$X$を$RR^N$の開集合として、$X$の二点を結ぶ$X$上の連続な曲線$c : [0, 1] arrow.r X$が存在するとする。
...
$d(x, y) < delta$が存在して\$\\abs{f(x)-f(y)} >= \\varepsilon\$である。
ここで$n = 1, 2, 3, dots$に対して$delta = n^(- 1)$として取れば、$x_n, y_n in X$が存在して$d(x_n, y_n) < n^(- 1)$,
\$\\abs{f(x\_n)-f(y\_n)} >= \\varepsilon\$が成り立つ。
$X$は有界閉集合なのでボルツァーノ・ワイエルシュトラスの定理より部分列を取れば数列$(x_n)$,
$(y_n)$は収束し極限をそれぞれ$x^(‾)$,
$y^(‾)$とおくと、$d(x_n, y_n) < n^(- 1) arrow.r 0$より$x^(‾) = y^(‾)$である。
さらに$x^(‾) = y^(‾) in X$もわかる。
ここで、$f$は$x^(‾) = y^(‾)$で連続より
\$\$\\varepsilon <= \\abs{f(x\_n)-f(y\_n)} \\to \\abs{f(\\bar{x})-f(\\bar{y})} = 0\$\$
