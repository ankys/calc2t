...
よって、$x$として$a_n$を考えることで、十分大きいすべての$n$に対して$lr(|f (a_n)-f (a)|) < epsilon$、つまり$f (a_n) -> f (a)$を得る。
...
+ $+ : x in RR, y in RR ->.bar x+y in RR$.

+ $- : x in RR, y in RR ->.bar x-y in RR$.

+ $times : x in RR, y in RR ->.bar x y in RR$.
...
+ 三角不等式より$r = sqrt((x-a)^2+(y-b)^2)$とおくと
  ...
  ここで、各$(a, b)$ごとに$lim_(r -> 0 +) (r^2+(lr(|a|)+lr(|b|)) r) = 0$より、乗法$times$は$(a, b)$で連続であり、$RR times RR$上の連続関数である。
...
+ 最初に$(a, b)$に十分近い$(x, y)$のみ考えることで$sqrt((x-a)^2+(y-b)^2) < lr(|b|) / 2$、特に$lr(|y-b|) <= lr(|b|) / 2$で$lr(|y|) >= lr(|b|) / 2$としてよいことに注意する。
  ...
  ここで、各$(a, b)$ごとに$lim_(r -> 0 +) (2 frac(r^2+lr(|b|) r, lr(|b|)^2)+r / lr(|b|)) = 0$より、除法\$\\divs\$は$(a, b)$で連続であり、\$RR\\times(RR\\setminus{0})\$上の連続関数である。
...
$ lim_(r -> 0 +) sup_theta lr(|f (a+r cos theta, b+r sin theta)-l|) = 0 $
が成り立つことである。

特に、$f$を$RR times RR$上で定義された二変数実数値関数とするとき、
二変数関数$f (x, y)$が$(x, y) = (a, b)$で連続であることの必要十分条件は
$ lim_(r -> 0 +) sup_theta lr(|f (a+r cos theta, b+r sin theta)-f (a, b)|) = 0 $
が成り立つことである。
...
を考える。 このとき
$ lr(|f (r cos theta, r sin theta)-f (0, 0)|) = lr(|r^2 cos theta sin theta|) / r = r lr(|cos theta sin theta|) <= r -> 0 quad(r -> 0) . $
...
#block[
最大について示す。
$f$の$X$上の上限を$l in RR^(‾)$とおくと、$X$が空集合でないので$l eq.not-oo$であり、$f (a_n) -> l$となる$X$上の点列$(a_n)$が存在する。
...
$y$に対して$x$と$y$を結ぶ$X$上の連続な曲線$c : [0, 1] -> X$が存在するとき、$X$は弧状連結であるという。
...
#block[
_Proof.]
曲線を$C$、$X$の補集合を$X^c$とおいて、関数$x in C, y in X^c ->.bar d(x, y)$が正の最小値を持つことを示せばよい。
...
$(y_n)$は収束し極限をそれぞれ$x^(‾)$,
$y^(‾)$とおくと、$d(x_n, y_n) < n^(- 1) -> 0$より$x^(‾) = y^(‾)$である。
さらに$x^(‾) = y^(‾) in X$もわかる。
ここで、$f$は$x^(‾) = y^(‾)$で連続より
