...
#block[
$f (x_1 , dots.h.c , x_N)$を$bb(R)^N$の部分集合$X$上で定義された$N$変数実数値関数、$i = 1 , dots.h.c , N$とする。

$(a_1 , dots.h.c , a_N)$を$X$の内点、つまり半径$r > 0$が存在して$B_r (a) subset X$が成り立つとする。
ここで他の成分は固定して第$i$成分だけ動かした時の微分係数の極限
$ lim_(x_i arrow.r a_i) frac(f (a_1 , dots.h.c , a_(i - 1) , x_i , a_(i + 1) , dots.h.c , a_N) - f (a_1 , dots.h.c , a_(i - 1) , a_i , a_(i + 1) , dots.h.c , a_N), x_i - a_i) $
が存在するとき$f$は点$(a_1 , dots.h.c , a_N)$で$x_i$について#emph[偏微分可能];であるといい、極限を#emph[偏微分係数];という。
関数$y = f (x_1 , dots.h.c , x_N)$の点$(a_1 , dots.h.c , a_N)$で$x_i$についての偏微分係数を
\$\$f\_{x\_i}(a\_1, \\cdots, a\_N),
\\quad {\\pdv{f}{x\_i}}(a\_1, \\cdots, a\_N),
\\quad \\eval{\\pdv{f}{x\_i}}\_{(x\_1, \\cdots, x\_N) = (a\_1, \\cdots, a\_N)}
\\quad \\eval{\\pdv{x\_i}(f(x\_1, \\cdots, x\_N))}\_{(x\_1, \\cdots, x\_N) = (a\_1, \\cdots, a\_N)},\$\$
\$\$\\eval{y\_{x\_i}}\_{(a\_1, \\cdots, a\_N)},
\\quad \\eval{\\pdv{y}{x\_i}}\_{(a\_1, \\cdots, a\_N)}\$\$ などで表す。

$X$を開集合、つまり$X$の点がすべて内点である場合を考える。
$X$のすべての点で関数$f$が$x_i$について偏微分可能であるとき$f$は$X$上$x_i$について偏微分可能であるという。
この時、$(x_1 , dots.h.c , x_N) in X$に対して偏微分係数$f_(x_i) (x_1 , dots.h.c , x_N)$を対応させる集合$X$上の実数値関数を$f$の$x_i$についての#emph[偏微分導関数];という。
$y = f (x_1 , dots.h.c , x_N)$の$x_i$についての偏微分導関数を
\$\$f\_{x\_i},
\\quad \\pdv{f}{x\_i},
\\quad \\pdv{x\_i}(f(x\_1, \\cdots, x\_N)),
\\quad y\_{x\_i},
\\quad \\pdv{y}{x\_i}\$\$ などで表す。

]
方向$x_i$と点$(a_1 , dots.h.c , a_N)$を決めるごとに偏微分係数は実数であり、
偏微分導関数は$X$上の実数値関数である。
偏微分導関数はしばしば#emph[偏導関数];と略される。
...
#block[
原点からの距離で定まる二変数関数 $ f (x , y) = sqrt(x^2 + y^2) $
の$x$偏微分を考える。
$y$を固定するごとに$x$について微分すると、$(x , y) eq.not (0 , 0)$においては偏微分可能で
$ f_x (x , y) = x / sqrt(x^2 + y^2) $ となる。
$(x , y) = (0 , 0)$においては$f (x , 0) = lr(|x|)$なので、これは$x$について偏微分可能でない。
...
#block[
<t:pdiff_non_cont> 例@t:non_contの二変数関数
...
がわかる。 $(x , y) = (0 , 0)$においても偏微分の定義から
$ f_x (0 , 0) = lim_(x arrow.r 0) frac(f (x , 0) - f (0 , 0), x) = 0 , quad f_y (0 , 0) = lim_(y arrow.r 0) frac(f (0 , y) - f (0 , 0), y) = 0 $
となり、偏微分可能である。
しかしながら、例@t:non_contで見た通りこの関数$f (x , y)$は$(x , y) = (0 , 0)$において連続ではない。
...
<全微分>
#block[
$N$変数関数$f (x) = f (x_1 , dots.h.c , x_N)$が点$x = a$において各$x_i$について偏微分可能であるとするとき、
偏微分係数を順番に並べて得られる$N$次元ベクトル
$ nabla f (a) = (f_(x_1) (a) , dots.h.c , f_(x_N) (a)) $
を$f (x)$の点$x = a$での#emph[勾配];という。
...
#block[
$f (x)$を$bb(R)^N$の部分集合$X$上で定義された$N$変数実数値関数、$a$を$X$の内点とする。
ここで、$f (x)$が点$x = a$で各方向に偏微分可能であって、勾配を使った極限の式
\$\$\\lim\_{x \\to a}\\frac{\\abs{f(x)-f(a)-\\nabla f(a)\\cdot(x-a)}}{\\norm{x-a}} = 0\$\$
が成り立つとき、関数$f (x)$は点$x = a$で#emph[全微分可能];であるという。
...
#block[
テキストによっては全微分
$ ⅆ f = f_(x_1) (a) ⅆ x_1 + dots.h.c + f_(x_N) (a) ⅆ x_N $
というものを導入することがある。
...
を考える。 この関数は$bb(R)^N$全体で全微分可能であり、勾配は
$ nabla f (x) = v $ となる。
...
#block[
#emph[Proof.] 三角不等式とコーシー・シュワルツの不等式より
\$\$\\abs{f(x)-f(a)} \\le \\abs{\\nabla f(a)\\cdot(x-a)}+\\norm{x-a}\\omega\_a(\\norm{x-a}) \\le \\norm{\\nabla f(a)}\\norm{x-a}+\\norm{x-a}\\omega\_a(\\norm{x-a}).\$\$
ここで$x arrow.r a$で最右辺は$0$に収束するので、$f (x)$は$x = a$で連続である。~◻
...
$f (x)$が多変数関数$f (x) = f (x_1 , dots.h.c , x_N)$の場合は$g (f (x))$も多変数関数で偏微分を考えると
\$\$\\pdv{x\_i}(g(f(x))) = g\'(f(x)){\\pdv{f}{x\_i}}(x),
\\quad \\nabla(g(f(x))) = g\'(f(x))\\nabla f(x)\$\$
となるが、問題は$g (y)$が多変数関数$g (y) = g (y_1 , dots.h.c , y_N)$の場合である。
このとき$f (x)$は$bb(R)^N$値関数$f (x) = (f_1 (x) , dots.h.c , f_N (x))$である必要があり、
$g (f (x))$は（$f (x)$が一変数関数ならば）一変数関数となるのでその微分を考える価値がある。
この関数が$x = a$で微分可能であるとは各$f_i (x)$が$x = a$で微分可能であることをいい、
$ f' (a) = (f_1 ' (a) , dots.h.c , f_N ' (a)) $ と定義する。

#block[
$bb(R)^N$値関数$f (x) = (f_1 (x) , dots.h.c , f_N (x))$が点$x = a$で微分可能で$N$変数関数$g (y) = g (y_1 , dots.h.c , y_N)$が点$y = f (a)$で全微分可能とする。
...
#block[
$bb(R)^N$の開部分集合$X$上の実数値関数$f$が各変数$x_i$に対して偏微分可能で導関数$nabla f$が$X$上の連続関数である時、$f$は$X$上#emph[連続微分可能];であるという。
...
#block[
$bb(R)^N$の開部分集合$X$上の実数値関数$f$が連続微分可能ならば、$f$は$X$上の各点で全微分可能である。

]
#block[
#emph[Proof.]
証明は点$a = (a_1 , dots.h.c , a_N) in X$とその近傍の点$x = (x_1 , dots.h.c , x_N) in X$について、$x$から$a$へ$x_1 , dots.h.c , x_N$軸に沿って近づけるという方法で行う。
...
ここで$x arrow.r.bar f (x_1 , dots.h.c , x , dots.h.c , x_N)$に対する平均値の定理より$0 lt.eq theta lt.eq 1$が存在して、
\$\$\\begin{aligned}
&\\abs{f(x\_1, \\cdots, x\_i, \\cdots, x\_N)-f(x\_1, \\cdots, a\_i, \\cdots, x\_N)-f\_{x\_i}(a\_1, \\cdots, a\_i, \\cdots, a\_N)(x\_i-a\_i)} \\\\
...
#block[
#emph[Proof.]
補題@t_pcurvより$X$の任意の二点$x , y$は$x_1 , dots.h.c , x_N$軸に並行ないくつかの線分で結ばれて、それぞれの線分で偏微分が$0$なので、$f (x) = f (y)$が成り立つ。~◻
...
#block[
$bb(R)^N$の開部分集合$X$上の関数$f$が変数$x_i$に対して偏微分可能で導関数$f_(x_i)$が各変数$x_j$で偏微分可能である時、$f$は$X$上二回偏微分可能であるといい、偏微分導関数
\$\$f\_{x\_i x\_j}, \\quad \\pdv{x\_j}(\\pdv{f}{x\_i}) = \\pdv{f}{x\_j}{x\_i}\$\$
を$f$の#emph[二階偏微分導関数];という。
同様にして、$i_1 , dots.h.c , i_n = 1 , dots.h.c , N$
($n = 1 , 2 , 3 , dots.h.c$)に対して偏微分導関数$f_(x_(i_1) dots.h.c x_(i_n))$を$f$の$n$階偏微分導関数という。
すべての$n$階偏微分導関数が連続の時、$f$は$X$上で#emph[$C^n$級];であるという。
さらに$f$が$X$上で任意の$n = 1 , 2 , 3 , dots.h.c$で$C^n$級である時、$f$は#emph[$C^oo$級];であるという。
...
#block[
#emph[Proof.] $h , k eq.not 0$として、平均値の定理を二回用いると
...
== ヤコビ行列とヘッセ行列
<ヤコビ行列とヘッセ行列>
$bb(R)^M$値関数に対して勾配ベクトルを並べて得られる行列をヤコビ行列という。

#block[
$N$次元点集合上の$M$個の実数値関数$f^1 (x) = f^1 (x_1 , dots.h.c , x_N) , dots.h.c , f^M (x) = f^M (x_1 , dots.h.c , x_N)$が内点$x = a$で偏微分可能な時、$bb(R)^M$値関数$F (x) = (f^1 (x) , dots.h.c , f^M (x))$は$x = a$で偏微分可能といい、
$N times M$型行列
\$\$J F(a) = \\nabla F(a) = \\mqty(f^1\_{x\_1}(a) & \\cdots & f^1\_{x\_N}(a) \\\\ \\vdots & \\ddots & \\vdots \\\\ f^M{x\_1}(a) & \\cdots & f^M\_{x\_N}(a))\$\$
を$F (x)$の$x = a$での#emph[ヤコビ行列];という。

特に$M = N$の場合はヤコビ行列は正方行列になり、
行列式$det J F (a)$が定義されて#emph[ヤコビ行列式];あるいは#emph[ヤコビアン];という。
...
#block[
$N$次元点集合上の実数値関数$f (x) = f (x_1 , dots.h.c , x_N)$が内点$x = a$の近傍で二回偏微分可能な時、$N$次正方行列
\$\$H f(a) = \\nabla^2 f(a) = \\mqty(f\_{x\_1 x\_1}(a) & \\cdots & f\_{x\_1 x\_N}(a) \\\\ \\vdots & \\ddots & \\vdots \\\\ f\_{x\_N x\_1}(a) & \\cdots & f\_{x\_N x\_N}(a))\$\$
を$f (x)$の$x = a$での#emph[ヘッセ行列];あるいは#emph[ヘシアン];という。
...
#block[
$N$次元開集合$X$上の関数$f$が内点$a$で$nabla f (a) = 0$を満たす時、$a$を$f$の#emph[停留点];という。
...
このような方程式から定まる関数のことを#emph[陰関数];という。
...
#block[
二変数実数値関数$f (x , y)$を点$(a , b) in bb(R) times bb(R)$の近傍で定義された$C^1$級関数で$f (a , b) = 0$とする。
...
ここで最右辺は$x , h$について有界なので、$h arrow.r 0$とすると$k arrow.r 0$であり、最右辺は収束するので最終的に
$ g' (x) = lim_(h arrow.r 0) frac(g (x + h) - g (x), h) = - frac(f_x (x , y), f_y (x , y)) = - frac(f_x (x , g (x)), f_y (x , g (x))) $
を得る。
...
#block[
#emph[Proof.]
$f$が$C^2$級の時、$f_x$と$f_y$は$C^1$級で定理より$g$も$C^1$級なので#link(<e:imdiff>)[\[e:imdiff\]];の両辺は$C^1$級であり、$g$は$C^2$級である。
...
しばしば境界が$g (x) = 0$の形をしているときに#emph[束縛条件];$g (x) = 0$のもとでの$f (x)$の最大最小を考えることになる。
