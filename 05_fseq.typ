...
より詳しくは$N = 0, 1, 2, 3, dots$に対して、第$N$項までの_部分和]
$
...
<= abs(f(x)-f\_n(x)}+abs(f\_n(x)-f\_n(a)}+abs(f\_n(a)-f(a)}
<= 2\\norm{f\_n-f}+\\omega\_{f\_n, a}(d(a, x))\$\$ が成り立つ。
よって、$epsilon > 0$に対して、十分大きい$n$を一つ取ることで\$\\norm{f\_n-f} <= \\frac{1}{3}\\varepsilon\$とでき、
...
\$\$abs(f\_n(x\_n)-f(x)}
<= abs(f\_n(x\_n)-f(x\_n)}+abs(f(x\_n)-f(x)}
<= \\norm{f\_n-f}+\\omega\_{f, x}(d(x, x\_n))
-> 0.\$\$ よって$f_n (x_n) -> f(x)$が成り立つ。
...
\$\$abs(f\_n(a\_n)-f(a\_n)} -> abs(f(a)-f(a)} = 0.\$\$
よって命題@t:cptfconvより、\$\\norm{f\_n-f} -> 0\$が従い、一様収束が言えた。
